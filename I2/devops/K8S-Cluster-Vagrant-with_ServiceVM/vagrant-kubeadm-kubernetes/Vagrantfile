...
Vagrant.configure("2") do |config|
  ...
  config.vm.define "services" do |services|
    ...
    services.vm.network "private_network", ip: settings["services"]["ip"]

    if settings["shared_folders"]
      settings["shared_folders"].each do |shared_folder|
        services.vm.synced_folder shared_folder["host_path"], shared_folder["vm_path"]
      end
    end

    services.vm.provider "virtualbox" do |vb|
      vb.cpus = settings["services"]["cpu"]
      vb.memory = settings["services"]["memory"]
      if settings["cluster_name"] and settings["cluster_name"] != ""
        vb.customize ["modifyvm", :id, "--groups", ("/" + settings["cluster_name"])]
      end
    end

    services.vm.provision "shell",
      env: {
        "DNS_SERVERS" => settings["network"]["dns_servers"].join(" ")
      },
      path: "scripts/services.sh"
  end
